...
Slide-level embeddings are needed for **discriminative tasks** (classification, biomarker prediction, survival prognosis, etc.). This requires modifying TRIDENT to support the slide encoder. We provide the necessary files under `./trident_modifications/`. Copy them into the corresponding directories in your TRIDENT installation:
 
```bash
cp -r ./trident_modifications/* /path/to/TRIDENT/
```
