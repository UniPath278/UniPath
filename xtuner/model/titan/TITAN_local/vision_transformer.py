...
# 将不规则分布的特征和坐标，映射到一个二维网格中，并返回网格化的特征、坐标以及背景掩码。
...
#dim: 输入特征的维度。
# num_heads: 注意力头的数量。每个头负责处理 dim / num_heads 的特征。
# qkv_bias: 是否为 qkv（查询、键和值）使用偏置。
# qk_norm: 是否对查询和键进行归一化。
# attn_drop: 注意力分数的丢弃率。
# proj_drop: 最终投影层的丢弃率。
# norm_layer: 用于归一化查询和键的层类型（默认为 nn.LayerNorm）。
# pos_encode: 位置编码类型（例如 'none', 'abs', 'alibi'）。
# 'alibi': 一种特殊的相对位置编码方法。
# 'abs': 绝对位置编码。
# 'none': 不使用位置编码。
...
#代码定义了一个名为 Block 的 Transformer 模块，用于实现标准的 Transformer 编码块，结合了注意力机制和多层感知机（MLP）。
...
class Block(nn.Module):
    ...
    def forward(self, x, attn_bias, bg_mask=None):
        ...
        return x
#类似于 PyTorch 的 nn.Sequential，但支持更灵活的前向传播逻辑。
#它的主要不同点在于，前向传播中支持额外的输入参数，例如 attn_mask 和 bg_mask，这些通常在注意力机制中被使用。
class CustomSequential(nn.Module):
    ...
    def forward(self, x, attn_mask, bg_mask=None):
        ...
        return x
#一个基于注意力的池化模块，用于从输入序列中提取关键特征。
#它通过自定义的查询向量（query）与输入特征序列（x）计算注意力权重，聚合重要信息。
#实现了基于多头注意力的特征池化，支持使用查询向量提取关键特征。
class AttentionalPooler(nn.Module):
    ...
    def forward(self, x: torch.Tensor, bg_mask: torch.Tensor = None,return_attn_weights=False):
        B = x.shape[0]
        if bg_mask is not None and B > 1:
            bg_mask = bg_mask.view(B, -1)
            bg_mask = torch.cat((torch.ones((B, 1), dtype=bg_mask.dtype, device=bg_mask.device), bg_mask), dim=-1)
        ...
        # if B == 1:
        #     out, attn_weights= self.attn(q.unsqueeze(1).expand(-1, N, -1), x, x, need_weights=True)
        # else:
        #     out, attn_weights = self.attn(q.unsqueeze(1).expand(-1, N, -1), x, x, attn_mask=bg_mask, need_weights=True)[0]
        # return out.permute(1, 0, 2).contiguous(), attn_weights # LND -> NLD
            # 基于B的不同值进行处理
        if B == 1:
            if return_attn_weights:
                out, attn_weights = self.attn(q.unsqueeze(1).expand(-1, N, -1), x, x, need_weights=True)
                return out.permute(1, 0, 2).contiguous(), attn_weights
            else:
            ...
            if return_attn_weights:
                out, attn_weights = self.attn(q.unsqueeze(1).expand(-1, N, -1), x, x, attn_mask=bg_mask, need_weights=True)
                return out.permute(1, 0, 2).contiguous(), attn_weights
            else:
                out = self.attn(q.unsqueeze(1).expand(-1, N, -1), x, x, attn_mask=bg_mask, need_weights=False)[0]
                return out.permute(1, 0, 2).contiguous()

#ViT 的核心思想是将图像切分为小的 patch，每个 patch 被嵌入到一个高维特征空间，然后通过 Transformer 层进行特征建模。
#在此实现中，VisionTransformer 模块基于网格 (grid) 大小定义输入特征空间的分辨率。
#patch_embed: 使用一个简单的两层全连接层替代传统的卷积层进行 patch embedding。
...
#这段代码定义了一个用于图像和嵌入处理的 forward 方法，
# 支持多种模型功能，例如位置编码、掩码训练（masked image modeling），以及不同类型的池化操作
    def forward_features(self, x, coords=None, mask=None, bg_mask=None):
        ...
        if self.pos_encode_type == 'alibi':
            if w*h == 36 and B != 1:
                ...
                #注意力偏置可以被加到相似度计算的结果中，从而影响模型的注意力分配
    ...
    def forward_attn_pool(self, x, bg_mask=None):
        ...
        tokens, attn_weights = self.attn_pool(x, bg_mask=bg_mask,return_attn_weights=True) 
        ...
        return pooled, tokens, attn_weights  # 返回第三个输出
    ...
    def get_alibi(self, w, h, bg_mask=None):
        ...
        # 将 x 和 y 转换为 PyTorch 张量并移至 GPU
        points = np.stack([x.ravel(), y.ravel()], axis=1)  # (w*h, 2)
        points_tensor = torch.tensor(points, dtype=torch.float32).to(device)
        
        # 计算 diffs 和 dists
        diffs = points_tensor[:, None, :] - points_tensor[None, :, :]
        dists = torch.sqrt(torch.sum(diffs ** 2, dim=-1))  # (w*h, w*h)
        ...
        # 获取 slopes，移动到 GPU
        slopes = torch.tensor(get_slopes(self.num_heads), dtype=torch.float32).to(device).view(self.num_heads, 1, 1)
        
        # 获取 dists 的形状并转换为 Tensor
        n_patches = dists.shape[-1]  # w*h 或 bg_mask.sum()
        dists_tensor = dists.to(device).view(1, n_patches, n_patches)
        
        # 计算 bias_matrix
        bias_matrix = dists_tensor * slopes * -1  # (1, num_heads, w*h, w*h)
        
        # 创建全 0 的张量并添加 bias_matrix
        embed_len = n_patches + 1
        all_bias = torch.zeros(1, self.num_heads, embed_len, embed_len).to(device)
        all_bias[:, :, 1:, 1:] = bias_matrix
