...
class LLaVAModel(BaseModel):
    ...
    def __init__(self,
        ...
        self.enable_titan = enable_titan
        if enable_titan:
            print('enable _titan')
        else:
            print('disable _titan')
        self.freeze_llm = freeze_llm
        self.freeze_visual_encoder = True
        if train_stage == '1':
            print('train_stage == 1')
            self.freeze_llm = True
            self.freeze_titan = True

        elif train_stage == '2':
            print('train_stage == 2')
            self.freeze_llm = True
            self.freeze_titan = False
        elif train_stage == '3':
            print('train_stage == 3')
        ...
        # self.titan = make_longnet_from_name(self.encoder_name) # , drop_path_rate=0.3, dropout=0.3, segment_length=1024
        self.titan = TitanVisionTower(
            config_path="./xtuner/model/titan/TITAN_local/config.json",
            weight_path="./xtuner/model/titan/TITAN_local/model_without_text_encoder.safetensors"
            #weight_path="/home/liangyuci/work_dirs/stage4-institution/epoch_1.pth/converted_model_not_pj_llm.safetensors"
        ...
        if self.freeze_titan:
            print('freeze_titan')
            self.titan.requires_grad_(False)
    ...
    def state_dict(self, *args, **kwargs):
        ...
        to_return.update(
            {k: v
             for k, v in state_dict.items() if 'titan.' in k})
        return to_return
    ...
    def forward(self, data, data_samples=None, mode='loss'):
        ...
        patch_size_lv0 = data.get("patch_size_lv0", None)
        ...
        if 'pixel_values' in data:
            ...
            if self.enable_titan:
                titan_output = self.titan(pixel_values, coords, patch_size_lv0) # shape: (576, img_num, 1024)
                # feat_to_proj = titan_output.permute(1, 0, 2) # shape: [1, img_num, 768]

            # pixel_values = self.projector(feat_to_proj.to(self.llm.dtype))
            pixel_values = self.projector(titan_output.to(self.llm.dtype))
            pixel_values = pixel_values.unsqueeze(0)
            data['pixel_values'] = pixel_values # shape: [1, 576, 4096]
            data.pop("coords", None)
            data.pop("patch_size_lv0", None)
    ...
    def to_hf(self,
        ...
        if save_format == 'xtuner':
            ...
            raise NotImplementedError
#主要是把训练好的各个子模块（LLM、视觉编码器、投影头、LongNet编码器）按照 XTuner/LLaVA 的结构分目录保存下来
    def to_xtuner_llava(self,
        ...
        # LongNet_encoder
        titan_path = osp.join(save_dir, 'titan')
        print_log(f'Saving titan to {titan_path}', 'current')
        self.titan.save_pretrained(titan_path,
                                       **save_pretrained_kwargs)
    ...
    def to_official_llava(self,
        ...
        titan_MAPPING = {
            'titan': 'model.vision_tower.vision_tower.titan_vision_model',
        }
        ...
        titan_state_dict = self.titan.state_dict()
        # titan_state_dict = convert_state_dict_to_hf(
        #     titan_state_dict, titan_MAPPING)
